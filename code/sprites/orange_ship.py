...
class OrangeShip(pygame.sprite.Sprite):
    def __init__(self, ground, num_moves, mountain_top):
        super(OrangeShip, self).__init__()
        self.surfs = [pygame.image.load(f"content/animations/orange_ship/{i}.png").convert_alpha() for i in range(3)]
        self.surfs = [pygame.transform.scale(i, (constants.orange_alien_width, self.surfs[0].get_height() * constants.orange_alien_width // self.surfs[0].get_width())) for i in self.surfs]
        ...
        self.death_surfs = [pygame.image.load(f"content/animations/ship_death/{i}.png").convert_alpha() for i in
                            range(5)]
        self.death_surfs = [pygame.transform.scale(i, (constants.orange_alien_width, i.get_height() * constants.orange_alien_width // i.get_width())) for i in
                            self.death_surfs]
